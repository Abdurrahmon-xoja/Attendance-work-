...
employees.forEach((emp, name) => {
  ...
  emp.days.forEach(day => {
    ...
    // Update rating cumulatively (like the bot does: currentRating + point)
  ...
  // Rating zone (using GREEN_ZONE_MIN=8, YELLOW_ZONE_MIN=5 from config)
  let ratingZone = '🔴 Риск';
  if (rating >= 8) {
    ratingZone = '🟢 Отлично';
  } else if (rating >= 5) {
  ...
  console.log(`${name}: Points=${totalPoints}, Rating=${rating.toFixed(1)}, Zone=${ratingZone}`);
  ...
  monthlyRows.push({
    ...
    'Last Updated': '2025-12-05 (CORRECTED v2)'
  });
});
...
console.log('\n✅ Corrected monthly report created (with fixed ratings)');
