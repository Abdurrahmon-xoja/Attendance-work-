...
const registrationWizard = new Scenes.WizardScene(
  ...
  async (ctx) => {
    ...
    if (!unregistered || unregistered.length === 0) {
      await ctx.reply(
        '❌ Вы не найдены в системе. Обратитесь к администратору.\n\n' +
        `Ваши данные:\n` +
  ...
  async (ctx) => {
    ...
    if (!employee) {
      await ctx.editMessageText('❌ Ошибка: сотрудник не найден. Попробуйте /start снова.');
      return ctx.scene.leave();
    }
    ...
    if (!workTime) {
      await ctx.editMessageText(
        '❌ Ошибка в расписании работы для этого сотрудника.\n' +
        'Обратитесь к администратору для исправления данных.\n\n' +
...
registrationWizard.action('confirm_registration', async (ctx) => {
  ...
  if (!employee) {
    await ctx.editMessageText('❌ Ошибка: данные не найдены. Попробуйте /start снова.');
    return ctx.scene.leave();
  }
...
registrationWizard.action('cancel_registration', async (ctx) => {
  ...
  if (!unregistered || unregistered.length === 0) {
    await ctx.editMessageText('❌ Регистрация отменена. Используйте /start для повтора.');
    return ctx.scene.leave();
  }
...
async function notifyAdminsAboutUnknownUser(ctx) {
  const notification =
    ...
    `• Username: @${ctx.from.username || 'не указан'}\n` +
    `• Имя: ${ctx.from.first_name || ''} ${ctx.from.last_name || ''}\n\n` +
    `Добавьте пользователя в Google Sheets (лист 'Roster')`;
...
function setupRegistrationHandlers(bot) {
  ...
  bot.command('help', async (ctx) => {
    const helpText =
      ...
      '• \'- сообщение\' - отметить уход (обязательно с сообщением!)\n' +
      '  Пример: \'- Иду домой\', \'- До завтра\'\n\n' +
      '🔹 ОПОЗДАНИЯ:\n' +
      `• Используйте кнопку '🕒 Опоздаю' до ${Config.LATE_DEADLINE_TIME}\n` +
      `• Льготное время: ${Config.GRACE_PERIOD_MINUTES} минут\n` +
      '• При опоздании нужно отработать дополнительное время\n\n' +
      '🔹 ДРУГИЕ ФУНКЦИИ:\n' +
