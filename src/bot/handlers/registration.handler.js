...
function setupRegistrationHandlers(bot) {
  ...
  bot.command('help', async (ctx) => {
    let helpText =
      ...
      '🔹 ОПОЗДАНИЯ:\n' +
      `• Пожалуйста, используйте кнопку '🕒 Опоздаю' в течение 15 минут после начала смены\n` +
      ...
      '🔹 РЕЙТИНГ (система баллов):\n' +
      `• Начальный балл за день: ${Config.BASE_POINTS}\n` +
      `• 🟢 Отлично: ≥ ${Config.EXCELLENT_ZONE_MIN}\n` +
      `• 🔵 Хорошо: ≥ ${Config.GOOD_ZONE_MIN}\n` +
      `• 🟡 Допустимо: ≥ ${Config.ACCEPTABLE_ZONE_MIN}\n` +
      `• 🟠 Плохо: ≥ ${Config.BAD_ZONE_MIN}\n` +
      `• 🔴 Недопустимо: < ${Config.BAD_ZONE_MIN}\n\n` +
      '🔹 ШТРАФЫ:\n' +
      `• Опоздание (предупредили до начала): -2\n` +
      `• Опоздание (без/после предупреждения): -4\n` +
      `• Ранний уход (10-40 мин): -4\n` +
      `• Ранний уход (40+ мин): -6\n` +
